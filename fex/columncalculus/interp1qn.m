...
% X, at each column of x. That is, y(l,m) interpolates Y(:,m), as a
% function of X(:,m), at x(l,m). No extrapolation is done: when x(l,n) is
...
% Y [M x N], the values of a function at the data sites
%
%
% --- Output:
...
%
% --- Code generation:
% INTERP1QN can be compiled into a MEX executible using codegen, as
% follows:
% >> codegen('interp1qn', '-args', {zeros(L,N), zeros(M,N), zeros(M,N)}, '-o', 'interp1qn_mex');
% To call the mex function rather than this, call interp1qn_mex().
...
% >> codegen('interp1qn', '-args', {type_x, type_X, type_X}, '-o', 'interp1qn_mex');
%
%
