function I = binsrchleftn(X,F) %#codegen
%BINSRCHLEFTN  Binary search for leftmost insertion index, many times.
%
%
% I = binsrchleftn(X,F) 
...
% (a) that binsrchleftn allows F to be multi-dimensional, whereas
%     discretize requires its 'edges' input to be a vector, and
% (b) binsrchleftn returns 1 or size(F,1) where X is outside the range of
%     F, whereas bdiscretize returns NaN here, and
% (c) binsrchleftn finds the leftmost insertion index, whereas discretize
%     finds the rightmost insertion index.
%
%
% --- Input:
...
%     codegen('binsrchleftn', '-o', 'binsrchleftn_mex', '-args', {type1, type2}, '-config', mexconfig);
