function [rho_s, rho_t] = eoscg_densjmd95_s_t(s,t,p) %#ok<FNDEF> %#codegen
%EOSCG_DENSJMD95_S_T  Fast salinity and potential temperature derivatives of JMD95 in-situ density.
%
%
% [rho_s, rho_t] = eoscg_densjmd95_s_t(s,t,p)  [kg m^-3 psu^-1, kg m^-3 degC^-1]
...
K_s =        eosJMDCKSw(1) + t.*(eosJMDCKSw(2) + t.*(eosJMDCKSw(3) + t.* eosJMDCKSw(4))) ...
    ...
    + p .* ( eosJMDCKP(5 ) + t.*(eosJMDCKP(6 ) + t.* eosJMDCKP(7)) + s1o2.*(1.5*eosJMDCKP(8)) ...
    + p .* ( eosJMDCKP(12) + t.*(eosJMDCKP(13) + t.* (eosJMDCKP(14))) ));
...
K_t =           (eosJMDCKFw(2) + t.*(2*eosJMDCKFw(3) + t.*(3*eosJMDCKFw(4) + t.*(4*eosJMDCKFw(5))))) ...
    ...
    + p .* (     eosJMDCKP(2 ) + t.*(2*eosJMDCKP(3) + t.*(3*eosJMDCKP(4))) ...
    +   s .*    (eosJMDCKP(6 ) + t.*(2*eosJMDCKP(7))) ...
    + p .* (     eosJMDCKP(10) + t.*(2*eosJMDCKP(11))  ...
