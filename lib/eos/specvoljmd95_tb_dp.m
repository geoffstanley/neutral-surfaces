function nu_p = specvoljmd95_tb_dp(s,t,p) %#ok<FNDEF> %#codegen
%SPECVOLJMD95_TB_DP  Fast derivative of the JMD95 specific volume w.r.t pressure.
%
%
% nu_p = specvoljmd95_tb_dp(s,t,p)                      [ m^3 / (dbar kg) ]
% computes the partial derivative of JMD95 specific volume with respect to
% pressure in dbar, given practical salinity s, potential temperature t,
% and pressure p.
%
%
% specvoljmd95_tb_dp is derived from densjmd95.m, documented below. Input
...
b =   eosJMDCKFw(1) + t.*(eosJMDCKFw(2) + t.*(eosJMDCKFw(3) + t.*(eosJMDCKFw(4) + t*eosJMDCKFw(5)))) ... % secant bulk modulus of fresh water at the surface
    ...
    + p .* b1plusb2; % secant bulk modulus of sea water at pressure p
