function dspecvol_dp = eoscg_gsw_specvol_v304_dp(SA,CT,p) %#ok<FNDEF> %#codegen
%EOSCG_GSW_SPECVOL_DP  Fast derivative of the GSW specific volume w.r.t. pressure.
%
%
% specvol_dp = eoscg_gsw_specvol_v304_dp(SA,CT,p)       [ m^3 / (dbar kg) ]
% computes the partial derivative of the GSW v3.04 specific volume with
% respect to pressure in dbar, given Absolute Salinity SA, Conservative
% Temperature CT, and pressure p.
%
%
% This function is derived from gsw_rho_first_derivatives.m, documented
...
rec_num = 1./v_hat_denominator;
       
specvol = rec_num.*v_hat_numerator;

dspecvol_dp = (dvhatnum_dp - dvhatden_dp.*specvol).*rec_num;
