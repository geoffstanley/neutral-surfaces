function rho_p = densjmd95_tb_dp(s,t,p) %#ok<FNDEF> %#codegen
%DENSJMD95_TB_DP  Fast derivative of the JMD95 in-situ density w.r.t pressure. 
%
%
% rho_p = densjmd95_tb_dp(s,t,p)                        [ kg / (dbar m^3) ]
% computes the partial derivative of the JMD95 in-situ density with respect
% to pressure in dbar, given practical salinity s, potential temperature t,
% and pressure p.                                         
% 
%
% densjmd95_tb_dp is derived from densjmd95.m, documented below. Input
...
b1plusb2 = b2 ...
    + eosJMDCKP(1) + t.*(eosJMDCKP(2) + t.*(eosJMDCKP(3) + t*eosJMDCKP(4))) ...        %\__ these 2 lines are b1
...
b =   eosJMDCKFw(1) + t.*(eosJMDCKFw(2) + t.*(eosJMDCKFw(3) + t.*(eosJMDCKFw(4) + t*eosJMDCKFw(5)))) ... % secant bulk modulus of fresh water at the surface
    ...
    + p .* b1plusb2 ; % secant bulk modulus of sea water at pressure z
...
% (b1 + 2 * b2) is b_p, the partial derivative of b w.r.t p
rho_p = rho .* (b - p .* (b1plusb2 + b2)) ./ (b - p).^2;
