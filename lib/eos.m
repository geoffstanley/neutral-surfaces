function rho = densjmd95_tb(s,t,p) %#ok<FNDEF> %#codegen
%DENSJMD95_TB  Fast JMD95 in-situ density.
%
%
% rho = densjmd95_tb(s,t,p)                                    [ kg / m^3 ]
% computes the JMD95 in-situ density given practical salinity s, potential
% temperature t, and pressure p.
%
%
% densjmd95_tb is derived from densjmd95.m, documented below. Input checks
...
%
% The inputs' and output's units are as in densjmd95, documented below.
% 
...
b =   eosJMDCKFw(1) + t.*(eosJMDCKFw(2) + t.*(eosJMDCKFw(3) + t.*(eosJMDCKFw(4) + t*eosJMDCKFw(5)))) ... % secant bulk modulus of fresh water at the surface
    ...
    + p .* ( b1 + b2 ); % secant bulk modulus of sea water at pressure z
