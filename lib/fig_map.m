...
if length(varargin) >= arg0+5  &&  isstruct(varargin{arg0+5})
    ...
    if isfield(OPTS, 'LANDCOL')
        maskcol = OPTS.LANDCOL;
    end
    if isfield(OPTS, 'NANCOL')
        nancol = OPTS.NANCOL;
    end
    if isfield(OPTS, 'GRIDCOL')
    ...
    if isfield(OPTS, 'LONTICK')
        lontick = OPTS.LONTICK;
    end
    if isfield(OPTS, 'LATTICK')
        lattick = OPTS.LATTICK;
    end
    if isfield(OPTS, 'LONLIM')
        lonlim = OPTS.LONLIM;
    end
    if isfield(OPTS, 'LATLIM')
        latlim = OPTS.LATLIM;
    end
