...
% loads the grid struct g for ECCO model data located the directory
% datafolder.
%
% [S, T, P, ETAN, ATMP, SAP] = load_ECCO2(datafolder, 'casts', timestep)
% loads the practical salinity S, the potential temperature T, the pressure
% P, the sea-surface height ETAN, the atmospheric pressure ATMP, and the
% Standard Atmospheric Pressure SAP at time timestep for ECCO model data
% located the directory datafolder.
%
% G = load_ECCO2(datafolder, 'gamma', timestep)
% loads the neutral density G at time timestep for ECCO model data
% located the directory datafolder.
%
% U = load_ECCO2(datafolder, 'u', timestep)
% loads the zonal velocity U at time timestep for ECCO model data
% located the directory datafolder.
%
% V = load_ECCO2(datafolder, 'v', timestep)
% loads the meridional velocity V at time timestep for ECCO model data
% located the directory datafolder.
%
% z = load_ECCO2(datafolder, 'omega', timestep, zref)
% loads the depth z of an omega surface computed from ECCO model data
...
% ATMP [1, 1]: zero. ECCO does not use atmospheric loading. [dbar]
% SAP  [1, 1]: zero. ECCO does not use additional Standard Atmospheric Pressure. [dbar]
