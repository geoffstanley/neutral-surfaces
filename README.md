Topobaric Surface - Software to compute topobaric surfaces and geostrophic streamfunctions
-------------------------------------------------------------------------------------------
...
>> cd('~/work/dphil/projects/topobaric_surface/src')
...
Step 2: Install recon.jar 
  ...
  If this is 1.7 or 1.8, continue to Step 3.
  Otherwise, you can compile ReCon yourself, or contact the author requesting an update.
  ...
  To compile ReCon yourself,
    ...
    Edit the text file PATH_TOPOBARIC_SURFACE/src/recon/build.xml
      Replacing each instance of "1.8" in the <javac ...> lines to specify MATLAB's JRE version.
      [
        Alternatively, run the following commands in bash, replacing ??? below with MATLAB's JRE version:
        JAVAVER=???
        sed -i "s/1\.8/$JAVAVER/g" $PATH_TOPOBARIC_SURFACE/src/recon/build.xml
      ]
...
Download the following files for the ECCO2 state estimate on Dec 22-24, 2002 by one of the following two methods:
  In a bash shell:
  Run the following commands, replacing the paths assigned to $ECCO2 and $OCCA with your own choices:
    ECCO2=~/work/data/ECCO2/latlon
    wget -P $ECCO/SALT/ ftp://ecco.jpl.nasa.gov/ECCO2/cube92_latlon_quart_90S90N/SALT.nc/SALT.1440x720x50.20021223.nc
    wget -P $ECCO/THETA/ ftp://ecco.jpl.nasa.gov/ECCO2/cube92_latlon_quart_90S90N/THETA.nc/THETA.1440x720x50.20021223.nc 
    wget -P $ECCO/UVEL/ ftp://ecco.jpl.nasa.gov/ECCO2/cube92_latlon_quart_90S90N/UVEL.nc/UVEL.1440x720x50.20021223.nc 
    wget -P $ECCO/VVEL/ ftp://ecco.jpl.nasa.gov/ECCO2/cube92_latlon_quart_90S90N/VVEL.nc/VVEL.1440x720x50.20021223.nc 
    wget -P $ECCO/SSH/ ftp://ecco.jpl.nasa.gov/ECCO2/cube92_latlon_quart_90S90N/SSH.nc/SSH.1440x720.20021222.nc
    wget -P $ECCO/SSH/ ftp://ecco.jpl.nasa.gov/ECCO2/cube92_latlon_quart_90S90N/SSH.nc/SSH.1440x720.20021223.nc
    wget -P $ECCO/SSH/ ftp://ecco.jpl.nasa.gov/ECCO2/cube92_latlon_quart_90S90N/SSH.nc/SSH.1440x720.20021224.nc
    ...
    echo PATH_TOPOBARIC_SURFACE:$PATH_TOPOBARIC_SURFACE >> $PATH_TOPOBARIC_SURFACE/run/paths.txt
    echo ECCO2:$ECCO2 >> $PATH_TOPOBARIC_SURFACE/run/paths.txt
    echo OCCA:$OCCA >> $PATH_TOPOBARIC_SURFACE/run/paths.txt

  Manually:
    In a web browser, go to ftp://ecco.jpl.nasa.gov/ECCO2/cube92_latlon_quart_90S90N/
    ...
    and similarly for others. If ~/work/data/OCCA/ is your base OCCA data directory, then the SALE data file should be in
