% Make most of the figures, using ECCO2 data, in 
% Stanley, G. J.  The exact geostrophic streamfunction for neutral
% surfaces. Ocean Modelling, submitted.
% and 
% Stanley, G. J.  Neutral surface topology. Ocean Modelling, accepted.
...
g.RAC = repmat(g.RACvec, g.nx, 1);
g.RAS = repmat(g.RASvec, g.nx, 1);
g.RAW = repmat(g.RAWvec, g.nx, 1);
g.YC = repmat(g.YCvec, g.nx, 1);
g.YG = repmat(g.YGvec, g.nx, 1);
rho_c = g.rho_c; 
grav = g.grav; 
Z = -g.RC(:); % We are going to be Z > 0 people!
Z2P = Pa2db * rho_c * grav ; % Note > 0
mk2d = @(F) reshape(F, [g.nx, g.ny]);
lead1 = @(x) reshape(x, [1 size(x)]);


%% Run codegen (Need only do this once)
run_codegen(g.nz, g.nx, g.ny, isvector(Z)); 

%% Get handles for MEX or MATLAB functions 
if exist('isopycnal_ijp_mex', 'file')
    isopycnal_ = @isopycnal_ijp_mex;
else
    isopycnal_ = @isopycnal;
end
if exist('deltasurf_mex', 'file')
    deltasurf_ = @deltasurf_mex;
else
    deltasurf_ = @deltasurf;
end
...
jp1 = @(F) subsasgn(circshift(F, [0 -1]), struct('type','()', 'subs', {{':',g.ny}}), nan); %#ok<SUBSASGN>
...
MLP = mixed_layer_pressure(S, T, Z * Z2P); % [dbar] 
MLZ = MLP / Z2P; % [m] 

%% Selecting surfaces and depths
list_surf = {'\sigma', '\delta', '\gamma^n', '\sigma_\nu', '\omega', '\tau', '\tau'''};
iSURF = containers.Map({'SIGMA', 'DELTA', 'GAMMA', 'ORTHO', 'OMEGA', 'TOPOB', 'MTOPO'}, 1:length(list_surf));
nS = length(list_surf);
...
mtopo_data = struct('RG', cell(nZ,1), 's0', cell(nZ,1), 't0', cell(nZ,1), 'rafnz', cell(nZ,1));
zs = nan(g.nx,g.ny,nS,nZ); % depth for each surface
...
OPTS.MLP = MLP;        % Mixed Layer Pressure
OPTS.TOL = tol;        % error tolerance when updating the surface
OPTS.ITER_L2_CHANGE = 1e-3; % Stop iterations when the L2 change of pressure on the surface is less than this
OPTS.ITER_MAX = 8;     % The maximum number of iterations
...
for iZ = 1 : nZ
    ...
    [zs(:,:,iSURF('SIGMA'),iZ), val] = isopycnal_(S, T, Z, zref, [i0, j0, z_i0j0(iZ)], tol, Z2P);
    ...
    hf = figure;
    ax = axes;
    fig_map(ax, g.XCvec, g.YCvec, -zs(:,:,iSURF('SIGMA'),iZ), land, rmfield(OPTS_FIGS, {'CLIM', 'CM'}))
    caxis([-zref-500 0]);
    colorbar
    ...
    hf = figure;
    ax = axes;
    fig_map(ax, g.XCvec, g.YCvec, -zs(:,:,iSURF('SIGMA'),iZ) - -zs(:,:,iSURF('OMEGA'),iZ), land, rmfield(OPTS_FIGS, {'CLIM', 'CM'}))
    caxis([-1 1] * 200);
    colorbar
    % colormap(ax, bluewhitered)
    ...
    [s,t] = interp1qn2_(lead1(zs(:,:,iSURF('OMEGA'),iZ)), Z, S, T);
    jj = y_to_j(-55) : y_to_j(-50);
    S_delta = nanmean(reshape(s(:,jj), [], 1));
    T_delta = nanmean(reshape(t(:,jj), [], 1));
    clear s t
    
    [zs(:,:,iSURF('DELTA'),iZ),val] = deltasurf_(S, T, Z, S_delta, T_delta, [i0, j0, z_i0j0(iZ)], tol, Z2P); 
    ...
    hf = figure;
    ax = axes;
    fig_map(ax, g.XCvec, g.YCvec, -zs(:,:,iSURF('TOPOB'),iZ) - -zs(:,:,iSURF('OMEGA'),iZ), land, rmfield(OPTS_FIGS, {'CLIM', 'CM'}))
    caxis([-1 1] * 60);
    colorbar
    % colormap(ax, bluewhitered)
...
[~, RG] = topobaric_surface(S, T, Z, zs(:,:,iSURF('SIGMA'),iZ), OPTS);

arc_ = zeros(RG.n_casts,1);
for e = 1:RG.nArcs
...
good_mask = false(g.nx,g.ny,1,nZ);
for iZ = 1:nZ
    ...
    good_mask(:,:,1,iZ) = select_conn_cpt(good_mask(:,:,1,iZ), g.WRAP, OPTS.REF_IJ);
...
eps = struct();
fdd = struct();
eps.mapx = nan(g.nx,g.ny,nS,nZ); % neutral error, zonal
eps.mapy = nan(g.nx,g.ny,nS,nZ); % neutral error, meridional
fdd.map  = nan(g.nx,g.ny,nS,nZ); % fictitious diapycnal diffusivity
...
for iZ = 1:nZ
    for iS = 1:nS
        ...
        [eps.mapx(:,:,iS,iZ), eps.mapy(:,:,iS,iZ)] = error_neutral(S,T,Z,z,g.DXCvec,g.DYCsc,false,g.WRAP,grav,rho_c);
        
        % Get slope errors on the Tracer grid
        [~, ~, sx, sy] = error_neutral(S,T,Z,z,g.DXCvec,g.DYCsc,true,g.WRAP,grav,rho_c);
...
hf = figure('Position', [0 0 600*2 400*nZ], 'Visible', 'off');
...
for iZ = 1 : nZ
    ...
    hf = figure('Position', [0 0 800 600], 'Visible', 'off');
...
OPTS_STRF = struct();
OPTS_STRF.WRAP = [true, false]; % For orthobaric Montgomery and topobaric gsf
...
ex = nan(g.nx,g.ny,nF,nSs,nZ); % zonal geostrophic velocity error
ey = nan(g.nx,g.ny,nF,nSs,nZ); % meridional geostrophic velocity error
...
for iZ = 1 : nZ
    for iS = 1 : nSs
        ...
        y     =     hsap2(S, T,     z_avg , Z, R, Y, grav, rho_c) ;
        y_adj = jm1(hsap2(S, T, jp1(z_avg), Z, R, Y, grav, rho_c));
        ...
        y     =     hsap2(S, T,     z_avg , Z, R, Y, grav, rho_c) ;
        y_adj = im1(hsap2(S, T, ip1(z_avg), Z, R, Y, grav, rho_c));
...
good_mask_ex = false(g.nx,g.ny,1,nSs,nZ);
good_mask_ey = false(g.nx,g.ny,1,nSs,nZ);
for iZ = 1:nZ
    for iS = 1:nSs 
        goodX = all(isfinite(ex(:,:,:,iS,iZ)), 3) | cori_V == 0;
        goodY = all(isfinite(ey(:,:,:,iS,iZ)), 3);
        goodX = select_conn_cpt(goodX, g.WRAP, OPTS.REF_IJ);
        goodY = select_conn_cpt(goodY, g.WRAP, OPTS.REF_IJ);
...
sv = .04; % spacing vert
hf = figure('Position', [0 0 400*nZ 266*nSs], 'Visible', 'off');
...
for iZ = 1:nZ
    ...
    for iS = 1:nSs
        ...
        if iS == 1
            ...
            text(ax, .3, 1.04-.05, '$\widetilde{\phantom{z}}$', 'Units', 'norm', 'FontSize', fs, 'Interpreter','latex');     % under z
        elseif iS == nSs
            y = ax.YLim(1) - diff(ax.YLim)*.11;
            y_ = ax.YLim(1) - diff(ax.YLim)*.15;
            for iF = cellfun(@(c) iSTRF(c), {'ZH', 'CU', 'MK', 'OM', 'TB', 'ZS'}) % all but AG
...
for iS = 1:nSs
    ...
    for iZ = 1:nZ
        ...
        hf = figure('Position', [0 0 800 round(200 * ceil(nF/2))], 'Visible', 'off');
        ...
        for iF =  1 : nF
            ...
            text(ax, 0, 1.06, ['$(' char('a' + iF-1) ')$'], ...
                'Units', 'norm', 'HorizontalAlignment', 'left', 'FontSize', fs+1, 'Interpreter', 'latex');
            text(ax, .07, 1.06, ['$\epsilon^{(x)} = ' list_geosu{iF} ' - u_g$'], ...
                'Units', 'norm', 'HorizontalAlignment', 'left', 'FontSize', fs+1, 'Interpreter', 'latex');
            text(ax, .07, .99, '$\phantom{\epsilon^{(x)} = }\, \widetilde{\phantom{u_g}}$', ...
                'Units', 'norm', 'HorizontalAlignment', 'left', 'FontSize', fs+1, 'Interpreter', 'latex');
            text(ax, .07, .99, ['$\phantom{\epsilon^{(x)} = ' list_geosu{iF} ' - } \, \widetilde{\phantom{u_g}}$'], ...
                'Units', 'norm', 'HorizontalAlignment', 'left', 'FontSize', fs+1, 'Interpreter', 'latex');
            text(ax, .98, 1.06, ...
        ...
        colorbar(ax, 'Position', [1-mr+.01 mb .0333 1-2*mb], 'FontSize', fs+1, ...
        ...
        ROI = select_conn_cpt(isfinite(z), g.WRAP, OPTS.REF_IJ);
        s_ = s(ROI); clear s
        t_ = t(ROI); clear t
        z_ = z(ROI);
        lat_ = g.YC(ROI);
        ...
        d_ = eos(s_, t_, z_ * Z2P) - eos(S_delta, T_delta, z_ * Z2P);
        ...
        text(ax, -.15, .7, '$z$  [m]', 'units', 'norm', 'interpreter', 'latex', 'fontsize', fs, 'Rotation', 90)
        text(ax, -.11, .7, '$\widetilde{\phantom{z}}$', 'units', 'norm', 'interpreter', 'latex', 'fontsize', fs, 'Rotation', 90)
...
% For this section, you must rename files so that eos.m computes specific volume! 

% Switch equation of state to the jmd95 specific volume 
copyfile([PATH_LOCAL '..' V 'lib' V 'eos' V 'specvoljmd95_tb.m'   ], [PATH_LOCAL '..' V 'lib' V 'eos.m'  ]);
copyfile([PATH_LOCAL '..' V 'lib' V 'eos' V 'specvoljmd95_tb_dp.m'], [PATH_LOCAL '..' V 'lib' V 'eosdp.m']);
clear eos eosdp % Make sure the copied file gets used
...
colorbar(ax, 'Position', [1-mr+.01 mb .0333 1-2*mb], 'FontSize', fs, ...
            'Ticks', cbticks, 'TickLabels', cblabel);
