...
mk3d = @(F) reshape(F, [size(F,1), nx, ny]);
...
OPTS.WRAP = WRAP;
OPTS.ITER_MAX = 0.5;  % Half an iteration, just to compute the Reeb Graph
...
lon_to_i = @(x) interp1(X, 1:nx, x, 'nearest');
lat_to_j = @(y) interp1(Y, 1:ny, y, 'nearest');
...
% Compute the Reeb graph, and simplify it
[p, RG] = topobaric_surface(S, T, P, p, OPTS);
ocean = RG.ocean;
...
% Re-order the arcs to keep colours visually separated
I = 1:RG.nArcs; 
clear e
e( 1) = find( RG.arc_from == find(node_label == 'c') ); % blue
e( 2) = find( RG.arc_from == find(node_label == 'A') ...
    &         RG.arc_to   == find(node_label == 'C') ); % pale blue
e( 3) = find( RG.arc_from == find(node_label == 'm') ); % dark green
e( 4) = find( RG.arc_from == find(node_label == 'b') ); % hot pink
e( 5) = find( RG.arc_to   == find(node_label == 'v') ); % maroon
e( 6) = find( RG.arc_from == find(node_label == 'a') ); % magenta
e( 7) = find( RG.arc_to   == find(node_label == 'y') ); % turqoise
e( 8) = find( RG.arc_to   == find(node_label == 'u') ); % dark blue
e( 9) = find( RG.arc_to   == find(node_label == 'x') ); % sky blue
e(10) = find( RG.arc_to   == find(node_label == 'r') ); % orange
e(11) = find( RG.arc_from == find(node_label == 'd') ); % red
e(12) = find( RG.arc_from == find(node_label == 'B') ...
    &         RG.arc_to   == find(node_label == 'G') ); % pale olive
e(13) = find( RG.arc_from == find(node_label == 'f') ); % green
I(e) = circshift(e,+1);
...
jig(:, node_label == 'a') = [    0    0   20    5    2 ];
jig(:, node_label == 'b') = [    0    0    0    5   -2 ];
jig(:, node_label == 'c') = [    0    0    0   -4   -3 ];
jig(:, node_label == 'd') = [    0    0  -10    3    2 ];
jig(:, node_label == 'e') = [    0   -3   10   -3    0 ];
jig(:, node_label == 'f') = [    0    0    0   -5   -2 ];
jig(:, node_label == 'g') = [    0    0  -70    5    0 ];
jig(:, node_label == 'h') = [    0    0    0   -4   -3e-1 ];
jig(:, node_label == 'j') = [    0    0  -10    3    0 ];
jig(:, node_label == 'k') = [    0    0  -10    3    0 ];
jig(:, node_label == 'm') = [    0    0  -10   -3    0 ];
jig(:, node_label == 'n') = [    0    0  -10   -3    0 ];
jig(:, node_label == 'p') = [    0    0    0    6    0 ];
jig(:, node_label == 'q') = [    0    0   -5    1    0 ];
jig(:, node_label == 'r') = [    0    0    5    2    0 ];
jig(:, node_label == 's') = [    0    0    5   -2    0 ];
jig(:, node_label == 't') = [    0    0    5    3    0 ];
jig(:, node_label == 'u') = [    0    0    5    1    0 ];
jig(:, node_label == 'v') = [    0    0    5    0    0 ];
jig(:, node_label == 'w') = [    0    0   -5   -7    0 ];
jig(:, node_label == 'x') = [    0    0    0    6    0 ];
jig(:, node_label == 'y') = [    0    0    0   -3    0 ];
jig(:, node_label == 'A') = [    0    0    0    7    0 ];
jig(:, node_label == 'B') = [    0    0    0   -5    0 ];
jig(:, node_label == 'C') = [    0    0    0    6    0 ];
jig(:, node_label == 'D') = [    0    0    0   -6    0 ];
jig(:, node_label == 'E') = [    0    0    5    4    0 ];
jig(:, node_label == 'F') = [    0    0   10   -4    0 ];
jig(:, node_label == 'G') = [    0    0   -7    3    0 ];
jig(:, node_label == 'H') = [    0    0   -7   -7    0 ];
jig(:, node_label == 'J') = [    0    0   10    2    0 ];
jig(:, node_label == 'K') = [    0    0  -12    0    0 ];
jig(:, node_label == 'M') = [    0    0  -12    1    0 ];
jig(:, node_label == 'N') = [    0    0   10    0    0 ];
jig(:, node_label == 'P') = [    0    0    7    5    0 ];
jig(:, node_label == 'Q') = [    0    0    0   -5    0 ];
jig(:, node_label == 'R') = [   -2    1   -7   -3    0 ];
jig(:, node_label == 'S') = [    0    0    0    5    0 ];
jig(:, node_label == 'T') = [    0    0   10   -4    0 ];
jig(:, node_label == 'U') = [    3    0   10    0    0 ];
jig(:, node_label == 'V') = [    0    0  -12    0    0 ];
jig(:, node_label == 'W') = [    0    0    0    9    0 ];

nodelonjig( node_label == 'j' ) = +2;
nodelonjig( node_label == 'P' ) = +3;
nodelonjig( node_label == 't' ) = +3;

% Just the casts indexed by RG.arc_segment. Gets rid of disconnected
% components on the surface, and ground.
p_ = p(ocean);

inds_ocean = find(ocean); % inverse to ij2v. a map from 2D space onto the 1D data vectors (p -> p_)
v2ij = @(v) ind2sub([nx ny], inds_ocean(v));
%v2i = @(v) mod1(inds_ocean(v), nx);  % function from a 1D data index to the i (longitude) in 2D space
%v2j = @(v) ceil(inds_ocean(v) / nx); % function from a 1D data index to the j (latitude) in 2D space
...
%{
a & 10\dege & 40\degn & 11 & C & A & 48\degw & 46\degn & 90 & ceC \\
b & 118\degw & 72\degs & 39 & B & B & 64\dege & 54\degs & 301 & dbG \\
c & 54\degw & 56\degn & 42 & A & C & 8\degw & 36\degn & 865 & aAD \\
d & 150\dege & 58\degs & 104 & B & D & 58\degw & 18\degn & 881 & fCpF \\
e & 74\degw & 78\degn & 283 & A & E & 8\dege & 14\degs & 891 & jhF \\
f & 80\degw & 30\degn & 583 & D & F & 18\degw & 2\degn & 892 & EDQ \\
g & 168\dege & 50\degn & 701 & H & G & 168\dege & 56\degs & 937 & BSQ \\
h & 34\degw & 16\degs & 838 & E & H & 146\dege & 16\degn & 991 & gwJ \\
j & 2\dege & 8\degs & 876 & E & J & 156\dege & 0\degs & 996 & kHsK \\
k & 158\degw & 16\degs & 944 & J & K & 114\degw & 2\degn & 998 & JrM \\
m & 108\degw & 20\degs & 972 & M & M & 82\degw & 4\degs & 999 & mKS \\
n & 74\dege & 16\degs & 987 & N & N & 74\dege & 10\degs & 1010 & nquP \\
p & 72\degw & 32\degn & 990 & D & P & 50\dege & 4\degn & 1011 & NtR \\
q & 94\dege & 10\degn & 994 & N & Q & 24\dege & 42\degs & 1023 & FGvT \\
r & 108\degw & 14\degn & 1007 & K & R & 42\dege & 22\degs & 1033 & PTU \\
s & 128\dege & 6\degn & 1046 & J & S & 76\degw & 54\degs & 1056 & GMV \\
t & 50\dege & 12\degn & 1029 & P & T & 34\dege & 28\degs & 1068 & RQU \\
u & 114\dege & 20\degs & 1031 & N & U & 50\dege & 20\degs & 1081 & RTV \\
v & 18\degw & 36\degs & 1184 & Q & V & 154\dege & 26\degs & 1154 & USW \\
w & 144\dege & 32\degn & 1244 & H & W & 158\dege & 44\degs & 1222 & Vyx \\
x & 156\degw & 48\degs & 1390 & W &  \\
y & 34\dege & 34\degs & 1410 & W &  \\
%}